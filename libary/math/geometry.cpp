...
{
	auto sp_1_x = start_point_1.x;
	auto sp_1_y = start_point_1.y;
	auto ep_1_x = end_point_1.x;
	auto ep_1_y = end_point_1.y;
	auto sp_2_x = start_point_2.x;
	auto sp_2_y = start_point_2.y;

	auto d = ep_1_x - sp_1_x;
	auto e = ep_1_y - sp_1_y;
	auto dist = float(sqrt(d * d + e * e));
	auto t1 = 0.f;
	auto s = abs(dist) > 0.f ? v_1 * d / dist : 0.f;
	auto k = abs(dist) > 0.f ? v_1 * e / dist : 0.f;

	auto r = sp_2_x - sp_1_x;
	auto j = sp_2_y - sp_1_y;
	auto c = r * r + j * j;
